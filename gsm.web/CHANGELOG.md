...
- Создайте проект React с помощью create-vite: `npm init --yes vite@latest gsm.web -- --template=react  --no-rolldown --no-immediate`.
- Обновление `vite.config.js` с портом.
- Создание файла проекта (`gsm.web.esproj`).
